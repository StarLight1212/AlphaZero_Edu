...
class SelfPlay:
    ...
    @staticmethod
    def _worker_func_wrapper(i):
        """静态包装函数，用于替代lambda函数，解决pickle问题"""
        # 这个函数会被传递给进程池，只需返回i即可，真正的worker_func会在之后调用
        return i
    ...
    def learn(self):
        ...
        if self.args.load_model:
            ...
            if os.path.exists(model_file):
                ...
                self.best_nnet.load_checkpoint(self.args.load_folder_file[0], self.args.load_folder_file[1])
            else:
        ...
        for iteration in range(1, self.args.num_iterations + 1):
            ...
            with mp.Pool(processes=self.num_processes) as pool:
                ...
                # 创建部分函数，包含所有需要的参数
                worker_func = partial(self._execute_episode_worker, self.game, self.nnet, self.args)

                # 并行执行自我对弈 - 使用静态包装函数而不是lambda
                results = []
                for i in tqdm(range(num_episodes), total=num_episodes, desc="并行自我对弈"):
                    # 首先获取索引，然后调用worker_func
                    episodes_result = worker_func()
                    results.append(episodes_result)
